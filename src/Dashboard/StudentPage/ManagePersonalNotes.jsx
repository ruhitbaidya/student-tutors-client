...
const ManagePersonalNotes = () => {
  ...
  const handeldelete = (id) => {
    secureApiCall.delete(`/deleteNote/${id}`).then((res) => {
      if (res.data) {
        refetch();
      }
    });
        ...
        <div>
          {notes?.map((item) => {
            return (
              <div
                key={item._id}
                className="bg-gray-100 rounded-lg mb-[10px] p-[20px]"
              >
                <div>
                ...
                <div>
                  <button
                    ...
                    className="py-[8px] w-full px-[30px] border bg-yellow-400 mt-[10px]"
                  >
                    Update
                  </button>
                  <button
                    onClick={() => handeldelete(item._id)}
                    className="py-[8px] px-[30px] border bg-red-400  w-full mt-[10px]"
                  >
                    Delete
                  </button>
                </div>
        ...
        <dialog id="my_modal_3" className="modal">
          <div className="modal-box">
            <h3 className="font-bold text-lg">
              {user.displayName} Update Your Note
            </h3>
            <form onSubmit={handelSubmits}>
              ...
              <div>
                <label className="form-control w-full">
                  ...
                  <textarea
                    defaultValue={siNote.noteDescription}
                    className="border border-gray-200 focus:outline-gray-200 rounded-lg p-[15px]"
              ...
              <div className="mt-[20px]">
                <button className="w-full py-[12px] bg-gray-300 rounded-lg">
                  Save
                </button>
              </div>
            </form>
