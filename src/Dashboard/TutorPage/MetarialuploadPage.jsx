...
const MetarialuploadPage = () => {
  ...
  const handelMeterialUpload = (e) => {
        ...
        axios.post(`https://api.imgbb.com/1/upload?key=210b46352f7d1f16f91db247e7e12194`, formdata)
        .then((res)=> {
           if( res.data.data.display_url){
            ...
            secureApiCall.post("/uploadMetrial", dataMeterial)
            .then((res)=> {
                setPageLoad(false)
                console.log(res)
        ...
        <div>
          <form onSubmit={handelMeterialUpload}>
            ...
            <div className="mt-[20px]">
                ...
                <button className="w-full py-[10px] bg-white rounded-lg">{pageload ?<span className="loading loading-spinner loading-xs"></span> : "Submit"}</button>
            </div>
          </form>
        </div>
      </div>
